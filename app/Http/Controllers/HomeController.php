...
class HomeController extends Controller
{
    ...
    public function subscribe(Request $request)
    {
        if($request->isMethod('post')){
            ...
            // echo "<pre>"; print_r($data); die;
            return redirect(url()->previous().'#subscribe')->with('subscribe_message','You are added to Subscribers List Successfully!');
        }
    }
    ...
    public function searchresult(Request $request)
    {
        ...
        $location_data = Location::where('l_id',$scityID)->first();
        if( $data['property_type'] == 1 || $data['property_type'] == 2 )
        {
            ...
            if(empty($scityID)){
                $properties = Property::where('offering_type','=',$url)->paginate(20);
                $type_name = Property::select('t_name')->where('offering_type','=',$url)->distinct()->get();
                $property_count = Property::where('offering_type', $url)->count();
            }else{
                $properties = Property::where('community','like', '%' . $location_data['community']. '%')->where('offering_type','=',$url)->paginate(20);
                $type_name = Property::select('t_name')->where('community','like', '%' . $location_data['community']. '%')->where('offering_type','=',$url)->distinct()->get();
                $property_count = Property::where('community','like', '%' . $location_data['community']. '%')->where('offering_type', $url)->count();
            }
        }
        else{
            $type = $data['property_type'];
                if(empty($scityID)){
                    $properties = Property::where('project_status','=','off_plan')->orderBy('created_at', 'desc')->paginate(20);
                    $type_name = Property::select('t_name')->where('project_status','=','off_plan')->distinct()->get();
                    $property_count = Property::where('project_status','=','off_plan')->count();
                }else{
                    $properties = Property::where('community','like', '%' . $location_data['community']. '%')->where('project_status','=','off_plan')->paginate(20);
                    $type_name = Property::select('t_name')->where('community','like', '%' . $location_data['community']. '%')->where('project_status','=','off_plan')->distinct()->get();
                    $property_count = Property::where('community','like', '%' . $location_data['community']. '%')->where('project_status','=','off_plan')->count();
                }
            }
            return view('frontend.property.property_category', compact('properties','scityID','type','property_count','type_name'));
        }
